...
const initPlanList = () => {
    ...
    planData.forEach(job => {
        initializeProgressCircle(`#progress-circle-${job.id}`, calculateJobProgress(job));
    });
};
...
const createJobElement = (job) => {
    ...
    const totalActivities = job.categories.reduce((count, category) => count + category.activities.length, 0);
...
const createCategoryElement = (category) => {
    ...
    categoryButton.setAttribute("data-bs-target", `#nestedCollapse${category.id}`);
  ...
  `;
    ...
    categoryCollapse.setAttribute("aria-labelledby", `nestedHeading${category.id}`);
    categoryCollapse.setAttribute("data-bs-parent", `#nestedAccordion${category.id}`);
...
const createActivityElement = (activity) => {
    ...
    activityElement.classList.add("activity-item", "d-flex", "align-items-center", "mb-2", "rounded-4");
